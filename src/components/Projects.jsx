...
const FILTERS = ['Tous', 'IA & Automation', 'Full-stack', 'Application', 'Jeu vidéo', 'Challenge', 'En cours']
...
export default function Projects() {
  ...
  return (
    <section id="projects" style={{ padding: 'clamp(4rem,8vw,6rem) clamp(1.5rem,8vw,6rem)', maxWidth: 1100, margin: '0 auto' }}>
      ...
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-80px' }} transition={{ staggerChildren: 0.1 }}>
        ...
        <motion.p variants={itemVariants} style={{ color: '#7a7890', fontWeight: 300, marginBottom: '2rem', maxWidth: 480 }}>
          Projets personnels, universitaires et professionnels — tous disponibles sur GitHub.
        </motion.p>

        {/* Filtres */}
        <motion.div variants={itemVariants} style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: '2.5rem' }}>
          {FILTERS.map(f => (
            ...
            {filtered.map((p, i) => (
              ...
              >
                ...
                <div style={{
                  ...
                  fontSize: '2.8rem', position: 'relative', background: p.gradient,
                }}>
                  {p.emoji}
                  <span style={{
                    ...
                    color: p.badgeColor, fontSize: 11, fontWeight: 600,
                ...
                <div style={{ padding: '1.25rem' }}>
                  <h3 style={{ color: '#e8e6f0', fontFamily: "'Syne',sans-serif", fontSize: '1rem', fontWeight: 600, marginBottom: 6 }}>{p.title}</h3>
                  <p style={{ color: '#7a7890', fontSize: 13, lineHeight: 1.6, marginBottom: '1rem' }}>{p.desc}</p>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: '1.25rem' }}>
                    {p.stack.map(s => (
                      <span key={s} style={{
                        ...
                        background: '#1a1a24', border: '1px solid rgba(255,255,255,0.07)', color: '#7a7890',
                      }}>{s}</span>
                    ))}
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 10 }}>
                    <a href={p.github} target="_blank" rel="noopener noreferrer" style={{
                      ...
                      fontSize: 12, color: '#7a7890', textDecoration: 'none',
                      padding: '6px 12px', borderRadius: 6,
                      border: '1px solid rgba(255,255,255,0.08)',
                      transition: 'color 0.2s, border-color 0.2s',
                    }}
                      onMouseEnter={e => { e.currentTarget.style.color='#e8e6f0'; e.currentTarget.style.borderColor='rgba(255,255,255,0.2)' }}
                      onMouseLeave={e => { e.currentTarget.style.color='#7a7890'; e.currentTarget.style.borderColor='rgba(255,255,255,0.08)' }}
                    >
                    ...
                    {p.demo && (
                      <a href={p.demo} target="_blank" rel="noopener noreferrer" style={{
                        display: 'inline-flex', alignItems: 'center', gap: 6,
                        fontSize: 12, color: '#7c6af7', textDecoration: 'none',
                        padding: '6px 12px', borderRadius: 6,
