...
export default function Navbar() {
  ...
  return (
    <header style={{
      ...
      {/* Logo */}
      <a href="#hero" aria-label="Accueil — Jérémie Henri" style={{
        gridColumn: '1', justifySelf: 'start',
        fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: '1.25rem',
        textDecoration: 'none', letterSpacing: '-0.03em',
        color: '#ffffff',
      }}>JH<span style={{ color: '#888888' }}>.</span></a>

      {/* Desktop nav */}
      <nav aria-label="Navigation principale" style={{ gridColumn: '2', justifySelf: 'center', display: 'flex', alignItems: 'center', gap: '0.5rem' }} className="hide-mobile">
        {LINKS.map(l => (
      ...
      <a href="#contact" className="hide-mobile" style={{
        ...
        padding: '9px 22px', borderRadius: 0, fontSize: 14, fontWeight: 700,
      ...
      <button
        ...
        style={{ gridColumn:'3',justifySelf:'end',background:'none',border:'none',color:'#f0eeff',cursor:'pointer',display:'none' }}
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.nav
            ...
            style={{ position:'absolute',top:'100%',left:0,right:0,background:'rgba(8,8,16,0.97)',backdropFilter:'blur(16px)',borderBottom:'1px solid rgba(255,255,255,0.06)',padding:'1.5rem clamp(1.5rem,5vw,3rem)',display:'flex',flexDirection:'column',gap:'0.75rem' }}
          >
            {LINKS.map(l => (
              <a key={l.href} href={l.href} onClick={()=>setOpen(false)} style={{ color:'#f0eeff',textDecoration:'none',fontSize:16,fontWeight:500,padding:'0.5rem 0' }}>{l.label}</a>
            ))}
            <a href="#contact" onClick={()=>setOpen(false)} style={{ marginTop:'0.5rem',padding:'12px',borderRadius:0,background:'#ffffff',color:'#080808',textDecoration:'none',textAlign:'center',fontWeight:600,fontSize:14 }}>
              Devis gratuit
            </a>
          </motion.nav>
        )}
